...
public class WaterPatch : MonoBehaviour
{
    ...
    public void UpdatePatch(float time)
    {
        ...
        for (int z = 0; z < _VertexLength; z++)
        {
            for (int x = 0; x < _VertexWidth; x++)
            {
                ...
                //_heightValues[index] = EstimateHeight(new Vector3(worldX, _heightValues[index], worldZ), time);
            }
        }
    ...
    {
        //return 0;
        //return _Ocean.TransformVertex(position, time).y;
        ...
        // y offset should not be based on zero
        xzOffset.y = 0;
